...
if [[ -z "$title" || -z "$status_name" || -z "$area_name" || -z "$type_name" ]]; then
  echo "Usage: $0 <title> <status> <area> <type> [body-file]"
  echo ""
  echo "  status:     Todo | In Progress | Done"
  echo "  area:       Frontend | Backend | Infra | Auth"
  echo "  type:       Spike | Feature | Task | Bug | Documentation"
  echo "  body-file:  path to a markdown file for the issue body (optional)"
  echo ""
  echo "Example: $0 \"My new task\" \"Todo\" \"Backend\" \"Spike\""
  echo "Example: $0 \"My new task\" \"Todo\" \"Backend\" \"Spike\" /tmp/issue-body.md"
  exit 1
fi
...
echo "Creating issue..."
if [[ -n "$body_file" ]]; then
  issue_url=$(gh issue create --repo "$repo" --title "$title" --body-file "$body_file" | tail -1)
else
